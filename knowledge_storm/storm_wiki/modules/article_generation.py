...
class StormArticleGenerationModule(ArticleGenerationModule):
    ...
    def generate_section(
        self, topic, section_name, information_table, section_outline, section_query, language="en"
    ):
...
class ConvToSection(dspy.Module):
    ...
    def forward(
        self, topic: str, outline: str, section: str, collected_info: List[Information], language: str = "en"
    ):
        info = ""
        ...
        with dspy.settings.context(lm=self.engine):
            section = ArticleTextProcessing.clean_up_section(
                self.write_section(topic=topic, info=info, section=section, language=language).output
            )
...
class WriteSection(dspy.Signature):
    ...
    IMPORTANT: Write the entire section in the specified language. If language is 'pt' (Portuguese), write in Portuguese. If 'en' (English), write in English.
    """
