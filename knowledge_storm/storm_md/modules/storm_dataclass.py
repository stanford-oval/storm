...
class StormInformationTable(InformationTable):
    ...
    def __init__(self, conversations=List[Tuple[str, List[DialogueTurn]]]):
        ...
        self.url_to_info: Dict[str, StormInformation] = StormInformationTable.construct_url_to_info(
            self.conversations
        )
    ...
    def construct_log_dict(
        ...
        conversation_log = []
        for persona, conv in conversations:
            conversation_log.append({"perspective": persona, "dlg_turns": [turn.log() for turn in conv]})
...
class StormArticle(Article):
    ...
    def insert_or_create_section(
        self, article_dict: Dict[str, Dict], parent_section_name: str = None, trim_children=False
    ):
        parent_node = (
            self.root if parent_section_name is None else self.find_section(self.root, parent_section_name)
        )
        ...
        for section_name, content_dict in article_dict.items():
            ...
            self.insert_or_create_section(
                article_dict=content_dict["subsections"], parent_section_name=section_name, trim_children=True
            )
    ...
    def update_section(
        ...
        self.insert_or_create_section(
            article_dict=article_dict, parent_section_name=parent_section_name, trim_children=False
        )

    def get_outline_as_list(
        self, root_section_name: Optional[str] = None, add_hashtags: bool = False, include_root: bool = True
    ) -> List[str]:
        """
    ...
    @classmethod
    def from_string(cls, topic_name: str, article_text: str, references: Optional[dict] = None):
