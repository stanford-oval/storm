...
class StormRunnerService:
    ...
    async def run_pipeline(
        ...
        if project_id in self.running_tasks:
            return {"status": "error", "message": "Pipeline already running for this project"}
    ...
    async def _run_pipeline_internal(
        ...
        try:
            ...
            try:
                from knowledge_storm import STORMWikiRunner, STORMWikiLMConfigs, STORMWikiRunnerArguments
                from knowledge_storm.storm_wiki.modules.callback import BaseCallbackHandler
                from knowledge_storm.lm import OpenAIModel
                from knowledge_storm.rm import TavilySearchRM, YouRM, DuckDuckGoSearchRM, SerperRM, GoogleSearch
            ...
            if config.retriever_type == "google" and (config.retriever_api_key or os.getenv('GOOGLE_SEARCH_API_KEY')):
                ...
                if cse_id:
                    rm = GoogleSearch(google_search_api_key=api_key, google_cse_id=cse_id, k=config.max_search_results)
            ...
            def run_pipeline():
                try:
                    ...
                    logger.info(f"Pipeline stages - Research: {config.do_research}, Outline: {config.do_generate_outline}, Article: {config.do_generate_article}, Polish: {config.do_polish_article}")
            ...
            if generated_content:
                ...
                self.file_service.update_project(project_id, {
