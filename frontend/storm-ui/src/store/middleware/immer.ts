...
export const createImmerActions = <T, A extends Record<string, (...args: any[]) => void>>(
  actions: (
...
export const immerHelpers = {
  // Add item to array
  addToArray: <T>(array: T[], item: T) => (draft: T[]) => {
    draft.push(item);
  },

  // Remove item from array by index
  removeFromArray: <T>(array: T[], index: number) => (draft: T[]) => {
    draft.splice(index, 1);
  },

  // Remove item from array by predicate
  removeFromArrayBy: <T>(predicate: (item: T) => boolean) => (draft: T[]) => {
  ...
  // Merge objects
  merge: <T extends Record<string, any>>(updates: Partial<T>) => (draft: T) => {
    Object.assign(draft, updates);
  },

  // Deep merge objects
  deepMerge: <T extends Record<string, any>>(updates: any) => (draft: T) => {
  ...
  resetProperties: <T extends Record<string, any>>(
    properties: (keyof T)[],
    defaults: Partial<T>
  ) => (draft: T) => {
  ...
  batch: <T>(...updaters: Array<(draft: Draft<T>) => void>) => (draft: Draft<T>) => {
