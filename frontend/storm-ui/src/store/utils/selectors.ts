...
export const createParametrizedSelector = <T, P, R>(
  ...
  return (params: P) => (state: T): R => {
    const key = JSON.stringify(params);
    const cached = cache.get(key);
    
    if (cached && Object.is(cached.lastState, state) && Object.is(cached.lastParams, params)) {
...
) => createSelector((state: T) => [...selector(state)].sort(compareFn), shallowEqual);
...
export const selectById = <T extends { id: string }>(id: string) =>
  (items: T[]) => items.find(item => item.id === id);

export const selectByIds = <T extends { id: string }>(ids: string[]) =>
  (items: T[]) => items.filter(item => ids.includes(item.id));

export const selectWhere = <T>(predicate: (item: T) => boolean) =>
...
export const pipe = <T, R1, R2>(
  selector1: (state: T) => R1,
  selector2: (result: R1) => R2
) => (state: T) => selector2(selector1(state));

export const compose = <T, R1, R2, R3>(
  selector1: (state: T) => R1,
  selector2: (result: R1) => R2,
  selector3: (result: R2) => R3
) => (state: T) => selector3(selector2(selector1(state)));

// Selector debugging utilities
export const debugSelector = <T, R>(
  selector: (state: T) => R,
