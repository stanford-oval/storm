...
export const debugApiKeys = () => {
  ...
  const envKeys = Object.keys(process.env)
    ...
    .reduce((acc, key) => {
      ...
      if (key.includes('KEY') || key.includes('TOKEN')) {
        acc[key] = value ? `${value.substring(0, 10)}...(${value.length} chars)` : 'NOT SET';
      } else {
  ...
  if (apiConfig) {
    ...
    console.log('Retriever Providers:', Object.keys(apiConfig.retrievers.providers));
    ...
    console.log('\nOpenAI Config:', openaiConfig ? {
      ...openaiConfig,
      apiKey: openaiConfig.apiKey ? `${openaiConfig.apiKey.substring(0, 10)}...` : 'NOT SET'
    } : 'NOT CONFIGURED');
    
    console.log('Tavily Config:', tavilyConfig ? {
      ...tavilyConfig,
      apiKey: tavilyConfig.apiKey ? `${tavilyConfig.apiKey.substring(0, 10)}...` : 'NOT SET'
    } : 'NOT CONFIGURED');
  } else {
