...
test.describe('STORM Pipeline Execution', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const projectCard = await projectPage.getProjectCard('Pipeline Test Project');
  ...
  test('should generate outline correctly', async ({ page }) => {
    ...
    await expect(page.getByTestId('outline-section')).toBeVisible({ timeout: 30000 });
  ...
  test('should generate article with citations', async ({ page }) => {
    ...
    await expect(page.getByTestId('article-content')).toBeVisible({ timeout: 60000 });
    
    // Verify article has content
    const articleContent = await page.getByTestId('article-content').textContent();
  ...
  test('should handle WebSocket connection for real-time updates', async ({ page }) => {
    ...
    const pipelineMessages = messages.filter(msg => 
      msg.type === 'pipeline_update' || msg.type === 'progress_update'
    );
  ...
  test('should resume interrupted pipeline after page refresh', async ({ page }) => {
    await pipelinePage.goto(projectId);
    await pipelinePage.startPipeline();
