...
async function createTestProject(page: Page) {
  ...
  await page.selectOption('[data-testid=lm-provider-select]', testConfig.languageModel.provider);
  await page.selectOption('[data-testid=lm-model-select]', testConfig.languageModel.model);
  await page.fill('[data-testid=temperature-input]', testConfig.languageModel.temperature.toString());
  
  // Configure retrieval settings
  await page.click('[data-testid=retrieval-tab]');
  await page.selectOption('[data-testid=retrieval-provider-select]', testConfig.retrieval.provider);
  await page.fill('[data-testid=max-results-input]', testConfig.retrieval.maxResults.toString());
...
async function waitForPipelineCompletion(page: Page, projectId: string, timeout = 300000) {
  let attempts = 0;
  const maxAttempts = timeout / 5000; // Check every 5 seconds
  
  while (attempts < maxAttempts) {
    const progressBar = page.locator('[data-testid=pipeline-progress]');
    const statusText = await progressBar.locator('[data-testid=status-text]').textContent();
...
test.describe('Critical User Flows', () => {
  ...
  test('Complete Project Creation and Management Flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=project-title]')).toContainText(testProject.title);
    await expect(page.locator('[data-testid=project-topic]')).toContainText(testProject.topic);
    ...
    await page.fill('[data-testid=duplicate-title-input]', 'Duplicated Test Project');
    ...
    await expect(page.locator('[data-testid=success-toast]')).toContainText('archived');
    
    // 7. Navigate back to dashboard and verify project is archived
    await page.goto('/dashboard');
    await page.click('[data-testid=show-archived-toggle]');
    await expect(page.locator(`[data-testid=project-card-${projectId}]`))
  ...
  test('Complete STORM Pipeline Execution Flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=pipeline-config-modal]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid=pipeline-progress-panel]')).toBeVisible();
    
    // Check research stage
    await expect(page.locator('[data-testid=research-stage-indicator]')).toHaveClass(/active|running/);
    await page.waitForSelector('[data-testid=research-conversations]', { timeout: 60000 });
    await expect(page.locator('[data-testid=research-conversations] .conversation-item')).toHaveCount(3);
    
    // 4. Review research results
    await page.click('[data-testid=research-tab]');
    await expect(page.locator('[data-testid=research-conversations]')).toBeVisible();
    ...
    await expect(firstConversation.locator('[data-testid=conversation-messages]')).toBeVisible();
    
    // 5. Wait for outline generation
    await expect(page.locator('[data-testid=outline-stage-indicator]')).toHaveClass(/active|running/);
    await page.waitForSelector('[data-testid=generated-outline]', { timeout: 120000 });
    ...
    await page.fill('[data-testid=new-section-description]', 'Extra insights on the topic');
    ...
    await expect(page.locator('[data-testid=article-stage-indicator]')).toHaveClass(/active|running/);
    
    // 8. Monitor article generation progress
    await page.waitForSelector('[data-testid=article-sections-progress]', { timeout: 180000 });
    
    // Check that sections are being generated
    await expect(page.locator('[data-testid=section-progress-0]')).toHaveAttribute('data-status', 'completed');
    ...
    await expect(articleContent.locator('[data-testid=citation-link]').first()).toBeVisible();
    
    // 10. Edit article content
    await page.click('[data-testid=edit-article-button]');
    await page.locator('[data-testid=article-editor] [contenteditable=true]').first().fill('Updated introduction paragraph');
    await page.click('[data-testid=save-article-button]');
    
    // 11. Complete pipeline with polish stage
    await page.click('[data-testid=continue-pipeline-button]');
    await expect(page.locator('[data-testid=polish-stage-indicator]')).toHaveClass(/active|running/);
    ...
    await expect(page.locator('[data-testid=pipeline-complete-banner]')).toBeVisible();
    ...
    await expect(stats.locator('[data-testid=word-count]')).not.toContainText('0');
    await expect(stats.locator('[data-testid=section-count]')).not.toContainText('0');
    await expect(stats.locator('[data-testid=citation-count]')).not.toContainText('0');
  });
  ...
  test('Article Export and Download Flow', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid=download-ready-notification]', { timeout: 60000 });
    ...
    await expect(page.locator('[data-testid=share-link-input]')).toHaveValue(/https:\/\/.*\/shared\/[a-f0-9-]+/);
  ...
  test('Error Handling and Recovery Flow', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=key-validation-error]')).toBeVisible();
    await expect(page.locator('[data-testid=key-validation-error]')).toContainText('invalid');
  ...
  test('Search and Filter Functionality', async ({ page }) => {
    ...
    await page.route('**/api/v1/projects*', route => {
      ...
      const allProjects = [
        { id: '1', title: 'AI Research Project', topic: 'Artificial Intelligence', status: 'completed' },
        { id: '2', title: 'Climate Change Analysis', topic: 'Environment', status: 'draft' },
        { id: '3', title: 'Machine Learning Guide', topic: 'Technology', status: 'in_progress' },
        { id: '4', title: 'AI Ethics Study', topic: 'Artificial Intelligence', status: 'completed' },
      ];
    ...
    await expect(page.locator('[data-testid=project-card]').first()).toContainText('AI Research Project');
    await expect(page.locator('[data-testid=project-card]').nth(1)).toContainText('AI Ethics Study');
  ...
  test('Responsive Design and Accessibility', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid=project-title-input]')).toBeFocused();
    
    await page.keyboard.press('Tab'); // Topic input
    await expect(page.locator('[data-testid=project-topic-input]')).toBeFocused();
    
    await page.keyboard.press('Tab'); // Description textarea
    await expect(page.locator('[data-testid=project-description-input]')).toBeFocused();
    
    // 4. Test screen reader accessibility
    // Check for proper ARIA labels and roles
    await expect(page.locator('[data-testid=project-title-input]')).toHaveAttribute('aria-label');
    await expect(page.locator('[data-testid=create-project-button]')).toHaveAttribute('aria-describedby');
...
test.describe('Performance and Load Testing', () => {
  test('Large Dataset Handling', async ({ page }) => {
    ...
    const itemCount = await page.locator('[data-testid=project-list-item]').count();
  ...
  test('Memory Usage and Resource Cleanup', async ({ page }) => {
    ...
    for (let i = 0; i < 5; i++) {
      await page.click('[data-testid=create-project-button]');
      await page.fill('[data-testid=project-title-input]', `Memory Test Project ${i}`);
